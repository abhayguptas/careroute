...
export class ScrapeRunRepository {
  static create(data: Omit<ScrapeRun, 'id' | 'startedAt'>): ScrapeRun {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.scraperId, data.snapshotId, data.status, data.recordCount, data.healthStatus, data.missingFields, startedAt);
...
export class HealingRepository {
  static create(data: Omit<HealingAttempt, 'id' | 'triggeredAt' | 'resolvedAt'>): HealingAttempt {
    ...
    db.prepare(`
      ...
      VALUES (?, ?, ?, ?, ?, ?, ?, ?)
    `).run(id, data.scraperId, data.prompt, data.status, data.fieldsBeforeHeal, data.fieldsAfterHeal, data.attempt, triggeredAt);

    return this.findById(id)!;
  }

  static findById(id: string): HealingAttempt | null {
    const attempt = db.prepare('SELECT * FROM healing_attempts WHERE id = ?').get(id) as HealingAttempt;
    return attempt || null;
  }
}
