...
export function EvidencePanel({ field, value, sourceUrl, evidenceText, lastChecked, className = '' }: EvidencePanelProps) {
  return (
    <div className={`bg-neutral-900 border border-border rounded-lg p-4 ${className}`}>
      <div className="flex justify-between items-start mb-3">
        <div>
          <h4 className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">{field}</h4>
          <div className="flex items-center gap-2">
            <span className="font-semibold text-white">{value}</span>
            <Badge variant="success" className="h-5"><CheckCircle2 size={12} className="mr-1" /> Verified</Badge>
          </div>
        </div>
        ...
        <div className="flex flex-col items-end gap-2">
          ...
          <a href={sourceUrl} target="_blank" rel="noreferrer" className="flex items-center text-xs text-[#0F766E] hover:text-[#0B5C55] transition-colors">
