...
import { Link as LinkIcon, Cpu, CheckCircle2, Database, AlertTriangle } from 'lucide-react';
...
export default function OnboardPage() {
        ...
        <div className="bg-neutral-950 border border-border rounded-2xl p-8 lg:p-12 shadow-sm text-center">
          ...
          <div className="relative w-24 h-24 mx-auto mb-8">
            {stage === 'done' ? (
              ...
              <>
                <div className="absolute inset-0 border-t-4 border-brand border-solid rounded-full animate-spin"></div>
                {stage === 'scraping' ? (
                  <Database className="absolute inset-0 m-auto text-brand" size={40} />
                ) : (
                  <Cpu className="absolute inset-0 m-auto text-brand" size={40} />
                )}
              </>
            )}
          </div>
  ...
  return (
    <div className="max-w-2xl mx-auto pt-8">
      ...
      <div className="bg-neutral-950 border border-border rounded-2xl p-8 shadow-sm">
        <form onSubmit={handleSubmit} className="space-y-6">
          <div>
            ...
            <input 
              ...
              className="w-full bg-neutral-900 border border-neutral-700 rounded-xl py-3 px-4 text-white placeholder-slate-600 focus:ring-2 focus:ring-brand focus:outline-none transition-all"
              required
            />
          </div>
